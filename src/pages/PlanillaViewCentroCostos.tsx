...
export default function PlanillaViewCentroCostos() {
  ...
  // --- NUEVAS FUNCIONES: EXPORTAR A EXCEL Y PDF ---
  const exportarExcel = () => {
    const datosMapeados = hojaActiva.rows.map((r: any) => ({
      "CÓDIGO": `SV-${r.id}`,
      "FECHA": r.fecha || '',
      "CONCEPTO / DESCRIPCIÓN": r.concepto || '',
      "CATEGORÍA": r.categoria || '',
      "RESPONSABLE": r.responsable || '',
      "MONTO DE GASTO": parseCurrency(r.monto)
    }));
  ...
  const exportarPDF = () => {
    ...
    // Títulos y KPIs
    doc.setFontSize(16);
    doc.text(`Centro de Costos - ${id?.replace('-', ' ').toUpperCase()}`, 14, 15);
    
    doc.setFontSize(10);
    doc.text(`Hoja: ${hojaActiva.nombre}`, 14, 23);
    doc.text(`Presupuesto: ${formatMoney(presupuestoActual)}`, 70, 23);
    doc.text(`Total Gastado: ${formatMoney(totalGastado)}`, 120, 23);
    doc.text(`Saldo: ${formatMoney(saldoDisponible)}`, 170, 23);
    ...
    doc.save(`Centro_Costos_${hojaActiva.nombre}.pdf`);
  };
  ...
  const columnas = useMemo(() => [
    ...
    { key: 'categoria', name: 'CATEGORÍA', renderEditCell: textEditor, width: 250, resizable: true, cellClass: (r: any) => getCellClass(r, 'categoria') },
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-100 overflow-hidden relative">
      ...
      {/* --- TOOLBAR --- */}
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center shrink-0 shadow-sm z-10 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2 md:gap-3">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
            ...
            <button onClick={exportarExcel} className="flex items-center gap-1.5 text-emerald-600 hover:bg-white px-3 py-1.5 rounded-lg text-sm font-bold transition-all" title="Exportar a Excel">
              <Download size={16} /> <span className="hidden lg:inline">Excel</span>
            </button>
          ...
          <button onClick={agregarFila} className="flex items-center gap-2 text-white px-4 md:px-5 py-2 text-sm font-bold rounded-xl shadow-md bg-emerald-600 hover:bg-emerald-700 transition-all hover:-translate-y-0.5"><Plus size={18} /> Gasto</button>
        </div>
      </div>
